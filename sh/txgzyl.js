...
async function GetBody() {
    if ($request && $request.url.indexOf("wxShopGift/draw") >= 0) {
        ...
        if (typeof $request.body !== 'undefined'){
            ...
            if (gzylBodyVal) $.setdata(gzylBodyVal, "jd_wxShopGiftId");
            $.log(
...
function requireConfig() {
  return new Promise(resolve => {
    ...
    const activityIdArrNode = $.isNode() ? process.env.jd_wxShopGiftId.split('@') : [];
    ...
    if ($.isNode()) {
      ...
      if ($.getdata('jd_wxShopGiftId')) $.activityIdArr = $.getdata('jd_wxShopGiftId').split('@').filter(item => !!item);
